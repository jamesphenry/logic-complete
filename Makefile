...
# Link main CPU executable
$(TARGET): $(ALL_OBJS)
	@mkdir -p $(dir $@)
	$(CC) $(ALL_OBJS) -o $@ $(LDFLAGS)
	@echo "Build successful: $@"

# Link test runner executable (links components + test code, excludes main.c)
$(TEST_TARGET): $(COMP_OBJS) $(TEST_OBJS)
	@mkdir -p $(dir $@)
	$(CC) $(COMP_OBJS) $(TEST_OBJS) -o $@ $(LDFLAGS)
...
# Compile test source files (includes tests/vendor for unity.h)
